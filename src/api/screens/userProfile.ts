...
export async function addUser(_: any, { user }: { user: User }): Promise<User> {
  try {
    const addedUser = await DbHelper.UserNode.AddUser(user);
...
export async function updateUser(_: any, { username, updatedUser }: { username: string; updatedUser: User }): Promise<void> {
  try {
...
export async function createPost(_: any, { post }: { post: Post }): Promise<Post> {
  try {
    const createdPost = await DbHelper.PostNode.CreatePost(post);
...
export async function addPlace(_: any, { place }: { place: Place }): Promise<void> {
  try {
    await DbHelper.PlaceNode.AddPlace(place);
    console.log(`Place added: ${place.id}`);
  } catch (error) {
    console.error(`Error adding place: ${error}`);
    throw error;
  }
}
export async function editPlace(_: any, { placeId, updatedPlace }: { placeId: string; updatedPlace: Place }): Promise<void> {
  try {
    await DbHelper.PlaceNode.EditPlace(placeId, updatedPlace);
    console.log(`Place edited: ${placeId}`);
  } catch (error) {
...
export async function createJourney(_: any, { journey }: { journey: Journey }): Promise<Journey> {
  try {
    const createdJourney = await DbHelper.JourneyNode.CreateJourney(journey);
    console.log(`Journey created: ${createdJourney.id}`);
...
export async function addReview(_: any, { review }: { review: Review }): Promise<Review> {
  try {
    const addedReview = await DbHelper.ReviewNode.AddReview(review);
...
export async function createCategory(_: any, { category }: { category: Category }): Promise<void> {
  try {
