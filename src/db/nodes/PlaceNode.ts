...
export class PlaceNode {
  ...
  public async AddPlaceToCategory(
    placeId: string,
    categoryName: string
  ): Promise<void> {
    try {
  ...
  // VISITED user place
  public async AddUserVisitedPlace(
    username: string,
    placeId: string
  ): Promise<void> {
    try {
  ...
  public async SearchPlace(place: string): Promise<Place[]> {
    try {
      ...
      return result.records.map(
        (record) => record.get("place").properties as Place
      );
    } catch (err) {
