...
export class NotificationNode {
  ...
  public async likeCommentNotification(
    noti: Notification
  ...
  public async getUserNotifications(username: string): Promise<Notification[]> {
    ...
    try {
      ...
      result.records.map((record) => {
        ...
        notification.initiatorsCntr =
          notificationNode.properties.initiators.length;
      ...
      console.error(
        `Error retrieving notifications for user ${username}: ${error}`
      );
      throw error;
    } finally {
      await session.close();
